...
export default class OldDashboard extends Component {
  render() {
    return (
      <div className="dashboard">
        <div className="control-bar">
          { user && <h1 className="greeting-text">{greeting}! <span>{user.name && user.name.first}</span></h1> }
                ...
                <div className="basic-info">
                  <h1 className="business-name">
                    ...
                    {this.state.editmode === "businessName" ? 
                      (<input type="text" className="info-input" onChange={(e)=>this.onChange(e)} name="businessName" value={this.state.business_name}></input>) : 
                      (<span className="info-input">{this.state.business_name}</span>)
                    }
                    {this.state.editmode === "businessName" ?
                  ...
                  <h4 className="business-address">
                    ...
                    {this.state.editmode === "address" ? 
                      (<input type="text" className="info-input" onChange={(e)=>this.onChange(e)} name="address" value={this.state.address.formatted_address}></input>) : 
                      (<span>{this.state.address.formatted_address}</span>)
                    }
                    {this.state.editmode === "address" ?
                    (<span type="button" className="btn btn-primary info-save" onClick={()=>this.setState({editmode: ''})}><FontAwesomeIcon icon="check"/>Save</span>) :
